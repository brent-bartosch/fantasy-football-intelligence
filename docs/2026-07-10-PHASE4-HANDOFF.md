...
- **ADR D7 gate is LIVE and mandatory**: any strategy/valuation change must pass `uv run python scripts/run_backtests.py --gate` (reference: composite 0.5297, band 0.0196, active row in `sim.backtest_reference`). Gate runs persist nothing; only `--reference` writes. **Gate caveat (Phase 3 final review): the 0.0196 band is season-dominated** — cross-season variance (2024 cells 0.55–0.58 vs 2023 0.48–0.51), not strategy variance, drives it. A calibration-induced change may trip the gate on season variance rather than real degradation: on a gate failure, check per-season deltas before concluding anything, and if the reference must be rebuilt after a legitimate data/calibration correction, document why.
...
8. **Small debt batch** (fold where convenient, from Phase 3 ledger + handoff review): `load_backtest_pool` ORDER BY; `data_vintage` per-position degraded fractions (bool_or over-flags); farm `git_sha` records parent commit on run-before-commit; PG_BIN eval alignment at next restore drill; **stale `adp_dd_ppr`/`pos_adp_dd_ppr` keys in `tests/test_sleeper_adapter.py` fixture data** (artifacts of the corrected Phase 3 handoff claim — replace with `adp_2qb`; production code is already correct).
...
- **`scripts/draft_assistant.py` is a DEAD v1-era assistant** (371 lines: direct psycopg2, the discarded flat-multiplier adjuster, 14-team assumptions — see PROJECT-RECORD §1 "effectively all v1 analytical code is dead"). The Phase 4 assistant REPLACES it; plan its retirement/archival explicitly rather than discovering it mid-build.
