...
## 13e. Addendum (2026-07-11): Phase 4 executed and merged

Phase 4 (Draft Assistant, Opponent Calibration & Rehearsal Ladder) executed subagent-driven — 17 tasks, per-task review gates (7 fix waves incl. 3 Critical catches), final whole-branch review "READY TO MERGE" (one launchd-chain finding fixed pre-merge), fast-forward merged to `main` at `1a950b3`. 468/468 tests; health gate extended to 30/30.

**Opponent calibration (R7 residual closed):** roster-state-conditioned `pos_need_scale` (QB (2.0, 1.5, 0.5)) adopted as `OpponentParams` default — sim opponents' QB1 mean 2.78→1.73 vs the room's historical 1.83 (QB2 4.50 vs 4.45; QB3 residual 8.86 vs 10.78 remains, direction: sim OVERSTATES 3rd-QB scarcity). Mechanism is conditional-persistence, NOT contagion (per the QB-run study). D7 gate PASSED post-adoption (composite 0.5330; no reference rebuild); sim-farm audit is now a hard uniform-sample regression check.

**Conclusions re-verified post-calibration:** "don't front-load QB" survives (strengthened); R7 sim-vs-backtest Spearman 0.20→0.60, above the 0.4 bar for the first time (seed-fragility caveat documented); QB1 opponent-bias caveat RESOLVED. New tier-target knob (`qb_tier_targets`): only (1,2,99) — QB1 from tier 1 (Josh Allen), QB2 from the 11-deep tier 2 — beats the round-plan control (+1.3pp, non-overlapping CIs). Caps now K:1/DEF:1 (second-kicker class dead; −0.39% A/B cost adjudicated a no-waivers evaluator artifact).

**Draft assistant live (`ffi/draft/` + `scripts/draft_assistant.py`, dead v1 deleted):** append-only fsync'd event log with atomic crash recovery; ADR-D1 mode machine (test-per-transition); Yahoo poller (diff-by-pick-number, log-before-return, crosswalk resolution) + proactive OAuth refresh; VONA availability layer (real-pool forecast 0.43s); recommendation engine structurally pinned to the sim strategy (`evaluate_rules` shared); single-log-handle + refuse-fresh-on-populated-log resume discipline; paper-board floor. FP news → typed signals (migrations 006–008) with human-confirmed capped adjustments (±10%/signal, ±20% cum) — `adjusted_pool` is live-board-only. Advisory agent lane hard-isolated in its own process.

**Rehearsal ladder armed:** drill harness (lag/999/refresh/crash vs written ADR D7 criteria) — ALL PASS at the 10s poll-interval ceiling (lag p95 7.87s < 15s; crash→resume state exactly equal); `rehearsal-1` tagged; draft-day + rehearsal-ladder runbooks committed with drill log. `draft-day` tag deliberately absent until a full Level-3 rehearsal passes.

**Pending user inputs (Phase 4 exit):** (1) private Yahoo test league for Level 2 (the only live-plumbing venue; before ~Aug 22 freeze); (2) 2026 league renewal/key (R8 re-audit trigger armed); (3) human half of the 999 drill at Level 1. Post-merge debt (triaged, pre-freeze): migration-tracking table; `get_session` revoked-token KeyError gap (startup-only, added to L2 drill checklist); per-day-cap first-adjustment race (single-operator, unreachable).
