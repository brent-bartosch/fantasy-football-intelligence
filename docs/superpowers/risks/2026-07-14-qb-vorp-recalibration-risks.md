...
| R4 | **Tier collapse neuters `qb_tier_targets`** — lower QB VORP compresses Boris-Chen QB tiers so `(1,2,99)` no longer meaningfully constrains; the search silently measures a different strategy than intended. | 6 | 6 | 36 | Log the QB tier distribution at each candidate rank before the search; assert ≥2 distinct QB tiers survive, else treat `qb_tier_targets` as inert and re-run with round-plan strategy. | At a candidate rank, all/most QBs land in one tier; `qb_tier_targets` picks identical to no-target run. |
...
| R6 | **Scenario vintage mismatch** — `qb_hoard_0`/`_24` materialized at a different projection/config snapshot than `qb_hoard_12`, so the measured win is partly a data artifact, not a baseline effect. | 4 | 7 | 28 | Before the search, assert all candidate scenarios share `config_version` and the same projection snapshot; rebuild all candidates from one vintage if not. | `SELECT DISTINCT config_version, snapshot` per scenario returns differing rows. |
